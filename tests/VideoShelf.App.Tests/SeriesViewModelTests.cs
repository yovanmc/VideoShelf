...
public class SeriesViewModelTests
{
    ...
    // ── D5 regression: SeriesCompleted transition guard ───────────────────────

    /// <summary>
    /// D5: SeriesCompleted fires exactly once on a genuine prev>0 → 0 transition.
    /// It must NOT fire on 0→0 repeats, nor when constructing a series that is
    /// already fully-watched (which would count as a false 0→0 on the first Refresh).
    /// </summary>
    [Fact]
    public void SeriesCompleted_fires_once_on_genuine_prev_gt0_to_0_transition()
    {
        using var temp = new AppTempDb();
        using var dir = new TempDir();
        var (lib, watch, sectionId) = Seed(temp, dir);
        ...
        var firedCount = 0;
        vm.SeriesCompleted += (_, _) => firedCount++;

        // Mark all watched and refresh → genuine N→0 transition: should celebrate.
        foreach (var e in lib.GetEpisodes(summary.SeriesId))
            watch.SetWatched(e.VideoId, true);
        vm.Refresh();
        ...
        // Refresh again with still-0 unwatched → 0→0: must NOT re-fire.
        vm.Refresh();

        firedCount.ShouldBe(1, "SeriesCompleted must not re-fire on 0→0 repeat Refresh");
    }

    [Fact]
    public void SeriesCompleted_does_not_fire_on_construction_of_already_complete_series()
    {
        using var temp = new AppTempDb();
        using var dir = new TempDir();
        var (lib, watch, sectionId) = Seed(temp, dir);
        ...
        // Pre-mark all episodes watched before constructing the VM.
        foreach (var e in lib.GetEpisodes(summary.SeriesId))
            watch.SetWatched(e.VideoId, true);

        // Re-fetch the summary so UnwatchedCount is already 0 at construction time.
        var completedSummary = lib.GetSeriesSummaries(sectionId).Single();
        completedSummary.UnwatchedCount.ShouldBe(0);

        var firedCount = 0;
        var vm = new SeriesViewModel(completedSummary, lib, watch, new StubThumbnailService());
        vm.SeriesCompleted += (_, _) => firedCount++;

        // Calling Refresh with an already-0 count must NOT trigger the event.
        vm.Refresh();

        firedCount.ShouldBe(0, "SeriesCompleted must not fire when series was already complete at construction");
    }
}
